# This framework has following integrations :
# connection to Snowflake Sales Fact Layer Source and Target Tables 
# utility to insert source STTM mapping queries via text file 
...
try :
    ...
    try:  
        ...
        for index,t in test.iterrows():
            ...
            spark = SparkSession \
                ...
                .config('spark.worker.timeout', '10000000')\
            ...
            #sqlContext = SQLContext(spark)   #Deprecated in 3.0.0 
    ...
    #Creating snowflake connection and loading source and target SF tables into CDL
    
    tgt_table_nm = sys.argv[4]   
    sfParams1={
    "sfURL": "",
    "sfUser": "",
    "sfjceks_alias": ".password.alias",
    "sfjceks_path": "/keystore/.jceks",
    "sfWarehouse": "",
    "sfdatabase": "",
    "sfschema": "",
    "usestagingtable": "OFF"
    ...
    jceks_file_path = "jceks://file/{jceks_path}".format(jceks_path=sfParams1["sfjceks_path"])
    sc = spark.sparkContext._jsc.hadoopConfiguration()
    sc.set("hadoop.security.credential.provider.path", jceks_file_path)
    sfParams1["sfpassword"] = ''.join(sc.getPassword(sfParams1["sfjceks_alias"]))
    
    print("--Making Connection for TARGET Snowflake table :",tgt_table_nm,"--")
    sfdf = spark.read.format("snowflake").options(**sfParams1).option("dbtable", tgt_table_nm).load()
    sfdf.createOrReplaceTempView("tgt_tbl")
    print("--Snowflake Connection to Target Created Succesfully--")
    
    src_table_nm = sys.argv[5] 
    src_table_nm_list = src_table_nm.split(",")
    print("--Making Connection for SOURCE Snowflake table(s) :",src_table_nm,"--")
    for tbl_nm in src_table_nm_list:
    
        if tbl_nm.lower().split(".")[0] == "dim":
        
            tbl_nm = tbl_nm.split(".")[1]
            sfParams2={
            "sfURL": "",
            "sfUser": "",
            "sfjceks_alias": ".password.alias",
            ...
            jceks_file_path = "jceks://file/{jceks_path}".format(jceks_path=sfParams2["sfjceks_path"])
            sc = spark.sparkContext._jsc.hadoopConfiguration()
            sc.set("hadoop.security.credential.provider.path", jceks_file_path)
            sfParams2["sfpassword"] = ''.join(sc.getPassword(sfParams2["sfjceks_alias"]))
            sfdf = spark.read.format("snowflake").options(**sfParams2).option("dbtable", tbl_nm).load() 
            ...
            sc.set("hadoop.security.credential.provider.path", jceks_file_path)
            sfParams1["sfpassword"] = ''.join(sc.getPassword(sfParams1["sfjceks_alias"]))
            sfdf = spark.read.format("snowflake").options(**sfParams1).option("dbtable", tbl_nm).load()
            
        sfdf.createOrReplaceTempView(tbl_nm)
        print("--Snowflake Connection for ",tbl_nm," Created Succesfully--")


    #Creating dataframe for testing's source query 
    
    testing_query = sys.argv[3]
    query = open(testing_query).read()
    source_table_df = spark.sql("{0}".format(query))
    source_table_df.createOrReplaceTempView("src_tbl")
    print("--Source Table Created Succesfully From Query--")
    ...
    def data_analysis(test_case_id,query1,query2,grain):
        ...
        src_only_df = src_only_df.persist()
        tgt_only_df = having_mismatches.where(t1['origin'].isNull())
        tgt_only_df = tgt_only_df.persist()
        ...
        logging.info("--Count of records in only source-- {0}".format(src_only))
        print("--Count of records in only source-- {0}".format(src_only))
        
        logging.info("---Done till 6c---")
        print("---Done till 6c---")
        
        logging.info("--Count of records in only target-- {0}".format(tgt_only))
        print("--Count of records in only target-- {0}".format(tgt_only))
    ...
    def finding_mismatch_columns(Test_case_id,having_mismatches,grain,col_list,t1,t2):
        ...
        if len(list(set(mismatches)))==1 and mismatches==[''] : 
            ...
            having_mismatches.createOrReplaceTempView('my_table')
